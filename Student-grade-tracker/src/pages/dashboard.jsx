...
const Dashboard = () => {
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            try {
                ...
                if (token) {
                    const userResponse = await axios.get('https://calm-dango-2d9f93.netlify.app/api/user', {
                        headers: {
                ...
                const gradesResponse = await axios.get('https://calm-dango-2d9f93.netlify.app/api/grades', {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const duplicateGrades = await Promise.all(subjects.map(async (subject) => {
                const response = await axios.get(`https://calm-dango-2d9f93.netlify.app/api/grades/exist`, {
            ...
            const newGrades = await Promise.all(subjects.map(async (subject) => {
                const response = await axios.post('https://calm-dango-2d9f93.netlify.app/api/grades', { 
                    moduleName: subject,
                    grade: formData[subject],
                    term: formData.term
                }, {
    ...
    const handleDelete = async (gradeId) => {
        try {
            const token = localStorage.getItem('token'); // Assume the token is stored in local storage
            await axios.delete(`https://calm-dango-2d9f93.netlify.app/api/grades/${gradeId}`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            });
